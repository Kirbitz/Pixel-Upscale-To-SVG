...
def update_image_frame(frame_increment=1):
    ...
    img = full_img[
        (currentFrame // sprites_per_row)
        * SPRITE_HEIGHT : ((currentFrame // sprites_per_row) + 1)
        * SPRITE_HEIGHT,
        (currentFrame % sprites_per_row)
        * SPRITE_WIDTH : ((currentFrame % sprites_per_row) + 1)
        * SPRITE_WIDTH,
    ]
...
def main():
    ...
    if not ANIMATED:
        update_image_upscale()
        cv2.createTrackbar(
            "Scale Type", gui_name, 0, 4, update_scale_type
